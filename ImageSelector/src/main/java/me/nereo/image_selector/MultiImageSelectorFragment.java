...
import me.nereo.image_selector.adapter.FolderAdapter;
import me.nereo.image_selector.adapter.ImageGridAdapter;
import me.nereo.image_selector.bean.Folder;
import me.nereo.image_selector.bean.Image;
import me.nereo.image_selector.utils.FileUtils;
import me.nereo.image_selector.utils.ScreenUtils;
        ...
        mGridView.setOnScrollListener(new AbsListView.OnScrollListener() {
            ...
            public void onScrollStateChanged(AbsListView view, int scrollState) {
                if (scrollState == SCROLL_STATE_FLING) {
                    Glide.with(view.getContext()).onStop();
//                    Picasso.with(view.getContext()).pauseTag(TAG);
                } else {
                    Glide.with(view.getContext()).onStart();
            ...
            if (intent.resolveActivity(getActivity().getPackageManager()) != null) {
                ...
                if (mTmpFile != null && mTmpFile.exists()) {
                    if (Build.VERSION.SDK_INT >= 24) {
                        intent.putExtra(MediaStore.EXTRA_OUTPUT, FileProvider.getUriForFile(getActivity(), getActivity().getPackageName()+".provider",
                                mTmpFile));
                    } else {
